      ...
      switch ($source_type) {
        case 'file_id_field':
          ...
          if (!empty($row->{$source_field})) {
            $fids[$row_index] = (int) $row->{$source_field};
          }
          continue;
        case 'file_field':
          ...
          if (!empty($row->_entity->{$source_field}) && $row->_entity->{$source_field} instanceof FieldItemListInterface) {
            $fids[$row_index] = $row->_entity->{$source_field}[0]->entity->id();
            $items[$row_index] = $row->_entity->{$source_field}[0]->entity;
          }
      }
    }
    if (empty($items)) {
