...
class LibrariesWebTest extends BrowserTestBase {
  ...
  public function testLibrariesScanInfoFiles() {
    ...
    dump('<pre>' . var_export(libraries_scan_info_files(), TRUE) . '</pre>');
  }
  ...
  public function _testLibrariesOutput() {
    ...
    $this->assertSession()->responseContains('libraries_test.js', 'Integration file loading: libraries_test.js found');
    $this->assertSession()->responseContains('libraries_test.css', 'Integration file loading: libraries_test.css found');
    $this->assertSession()->responseContains('libraries_test.inc', 'Integration file loading: libraries_test.inc found');
    ...
    foreach ($names as $name => $expected) {
      foreach ($extensions as $extension) {
        ...
        if ($expected) {
          ...
          $this->assertSession()->responseContains($raw, "$label$name.$extension found.");
        }
        else {
          $html_not_expected[] = Html::escape($raw);
          $this->assertSession()->responseNotContains($raw, "$label$name.$extension not found.");
        }
      }
    }
