...
class ForceLoginController implements ContainerInjectionInterface {
  ...
  public function forceLogin(Request $request) {
    ...
    // Remove the "destination" parameter from the request if present.
    // Without doing so, Drupal will redirect directly to this destination
    // instead of to the CAS server. The destination param is not lost, it has
    // been saved in $cas_redirect_data and it becomes part of the CAS service
    // URL that we redirect to.
